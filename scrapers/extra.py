...
class WaTimesScraper(BaseScraper):
    ...
    def _detail_links(self, cat_path):
        url = self.BASE + cat_path + ".html"
        r = requests.get(url, headers=UA, timeout=45)
        r.raise_for_status()
        # detail pages look like category/<id>/<slug>/listings/<set>/<num>.html
        links = re.findall(r'category/\d+/[A-Za-z0-9-]+/listings/\d+/\d+\.html', r.text)
        seen = []
        for l in links:
            if l not in seen:
                seen.append(l)
    ...
    def fetch(self, max_pages=1):
        for cat_path, county, state in self.CATEGORIES:
            ...
            for link in links:
                try:
                    dr = requests.get(self.BASE + link, headers=UA, timeout=45)
                    dr.raise_for_status()
                except Exception:
                    continue
                ...
                yield Notice(
                    ...
                    property_address=parsed.get("property_address"),
                    court_location=parsed.get("court_location"),
                    county=county or parsed.get("county"), state=state,
                    title=text[:140], full_text=text[:4000],
                    url=self.BASE + link,
                )
